...
def NMS_bad(outputs, conf_thresh, iou_thresh):
    
    output_nms = [] 
    
    for output in outputs:
        
        good_boxes = {'boxes': [], 'labels': [], 'scores': []}
        ...
        'if there is only one box, leave it'
        if sum(mask) == 1:
            for key, arr in zip(good_boxes.keys(), [boxes, labels, scores]):
                good_boxes[key].append(arr[0])
            continue
        
        'check every pair of remaining boxes'
        for i in range(len(boxes) - 1):
            for j in range(i + 1, len(boxes)):
                ...
                if iou < iou_thresh:
                    for key, arr in zip(good_boxes.keys(), [boxes, labels, scores]):
                        good_boxes[key].append(arr[i])
                        good_boxes[key].append(arr[j])
                else:
                    k = max(i, j, key=lambda x: scores[x])
                    for key, arr in zip(good_boxes.keys(), [boxes, labels, scores]):
                        good_boxes[key].append(arr[k])
                    
        output_nms.append(good_boxes)
